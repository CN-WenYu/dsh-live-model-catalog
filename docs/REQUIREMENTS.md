...
| 代码审查（冗余清理） | 逐模块统计导出与内部实现的使用次数后，移除三处冗余：`lib/index.js` 的 `targetsNow`（发现范围改造后已无调用者，且它留下的 `resolvedTargets` 间接层只剩一个调用点，已内联）、`lib/listing.js` 的 `numberOf`（定义后全树零引用）、以及三份逐字相同的「取第一个非空字符串」helper（`routes.firstText` / `translate.text` / `listing.label`，合并为 `constants.firstText`）；`listing.js` 的 `MAX_RESPONSE_BYTES` 与 `USER_AGENT` 仅在本模块使用，已取消导出。`lib/` 由 2163 行降至 2150 行，`npm test` 前后同为 **163 通过**（行为未变，纯清理） |
