...
export function apply(ctx) {
   ...
   *
   * The write half of the round resolves its own targets against the raw user
   * layer it just read (`doSync`), so this is the only scope resolved from the
   * ambient settings here.
   */
  const discoveryTargetsNow = async (cfg) => {
    const resolved = await endpointTable();
    return resolveDiscoveryTargets({ config: cfg, llmUser: llmUser(), builtin: resolved.table, catalog: resolved.models });
  };
