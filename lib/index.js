...
import { resolveDiscoveryTargets, resolveTargets } from './routes.js';
...
export function apply(ctx) {
  ...
  const resolvedTargets = async (cfg, resolve) => {
    const resolved = await endpointTable();
    return resolve({ config: cfg, llmUser: llmUser(), builtin: resolved.table, catalog: resolved.models });
  };

  /** Routes this plugin may write to: only the ones the owner declared. */
  const targetsNow = (cfg) => resolvedTargets(cfg, resolveTargets);

  /**
   * Routes this plugin may answer discovery for: the declared ones plus every
   * provider pi-ai ships, declared or not. The wider scope is what makes the
   * Models page's button live for a built-in provider that has not been adopted
   * yet — the state in which that button is actually used.
   */
  const discoveryTargetsNow = (cfg) => resolvedTargets(cfg, resolveDiscoveryTargets);
  ...
  const syncDiscovery = async (cfg) => {
    managedRoutes.clear();
    for (const target of await discoveryTargetsNow(cfg)) if (target.enabled) managedRoutes.add(target.route);
    ...
    if (cfg.enabled === false || cfg.fixDiscovery !== true) {
      return { ...(await removeDiscovery({ ctx })), scope: managedRoutes.size };
    }
    return {
      ...(await ensureDiscovery({
        ...
        live: async (provider, request, signal) => {
          const target = (await discoveryTargetsNow(config())).find((item) => item.route === provider);
          // A protocol DSH itself refuses to read has no listing to ask for:
          // naming the wrong URL with the wrong auth would replace the platform's
          // precise "enter this provider's models by hand" with a confusing 404.
          if (target === undefined || !target.listable) return undefined;
