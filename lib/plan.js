...
export function planRouteUpdate({ target, config, live, include, since, user, additions = true }) {
  ...
  const common = {
    ...
    // The owner's declaration for this route. The endpoint's own answer outranks
    // it, and it is the only source for a provider whose listing publishes no
    // reasoning metadata at all.
    routeEfforts: target.efforts,
