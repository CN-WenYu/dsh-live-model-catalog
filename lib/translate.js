...
export function effortMap(reasoning, fallbackEfforts = FALLBACK_EFFORTS) {
  ...
  if (declared !== undefined && declared.length > 0) {
    ...
    for (const [level, wire] of Object.entries(fallbackEfforts ?? {})) {
      if (!THINKING_LEVELS.includes(level)) continue;
      if (wire === null) efforts[level] = null;
      else if (typeof wire === 'string' && wire.length > 0) efforts[level] = wire;
    }
    if (Object.keys(efforts).length > 0) notes.push('no effort list published; preset applied');
  }
