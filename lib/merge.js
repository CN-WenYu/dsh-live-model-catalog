 ...
 *   set (or deliberately left to inherit) stays exactly as it is;
...
import { isAlias, translateEntry } from './translate.js';
...
export function planRoute({
  ...
  for (const entry of current) {
    ...
    if (liveEntry === undefined) {
      notAdvertised.push(id);
      next.push(entry);
      continue;
    }

    const { profile, notes } = translateEntry(liveEntry, { fallbackEfforts });
    const patches = {};
